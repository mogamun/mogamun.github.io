...
const HeroTitle = styled.h1`
  ...
  .line1 { display: block; opacity: 0; animation: ${slideUp} 0.8s cubic-bezier(0.16,1,0.3,1) 0.5s forwards; }
  .line2 { display: block; opacity: 0; animation: ${slideUp} 0.8s cubic-bezier(0.16,1,0.3,1) 0.75s forwards; }
`;
const HeroSubtitle = styled.p`
  font-size: clamp(0.95rem, 2.2vw, 1.15rem);
  color: ${({ theme }) => theme.colors.gray};
  margin: 0 auto 32px; max-width: 520px; line-height: 1.7;
  opacity: 0; animation: ${slideUp} 0.8s cubic-bezier(0.16,1,0.3,1) 1s forwards;
`;
...
const CTAButton = styled.a`
  ...
  ${({ $primary, theme }) => $primary ? css`
    ...
    &:hover { transform: translateY(-2px); box-shadow: 0 8px 30px ${theme.colors.glowPrimary}; opacity: 1; }
  ` : css`
    background: ${theme.colors.glass}; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
    border: 1px solid ${theme.colors.glassBorder}; color: ${theme.colors.headline};
    &:hover { background: ${theme.colors.glassBorder}; transform: translateY(-2px); opacity: 1; }
  `}
`;
...
const SectionTitle = styled.h2`
  font-size: clamp(1.4rem, 4vw, 2rem); font-weight: 800; margin: 0;
  color: ${({ theme }) => theme.colors.headline};
...
const AppCardGrid = styled.div`
  ...
  gap: 20px;
`;
const AppCardWrap = styled.div` will-change: transform; `;
...
const AppCardIcon = styled.div`
  ...
  &::after {
    ...
    animation: ${shimmerAnim} 3s ease infinite; opacity: 0;
  }
  ${AppCardInner}:hover &::after { opacity: 1; }
`;
const AppCardContent = styled.div` padding: 16px 18px; `;
const AppCardTitle = styled.h3`
  font-size: 1rem; font-weight: 700; margin: 0 0 4px;
  color: ${({ theme }) => theme.colors.headline};
`;
const AppCardDesc = styled.p`
  font-size: 0.8rem; color: ${({ theme }) => theme.colors.gray}; margin: 0; line-height: 1.4;
`;

function AppCard({ to, icon, title, desc }) {
  const { ref, handleMouseMove, handleMouseLeave, handleMouseEnter } = useTilt(5);
  return (
    <AppCardWrap ref={ref} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} onMouseEnter={handleMouseEnter}>
      <AppCardInner to={to}>
...
const FeaturedWrapper = styled.div`
  ...
  &::before { left: 0; background: linear-gradient(to right, ${({ theme }) => theme.colors.background}, transparent); }
  &::after  { right: 0; background: linear-gradient(to left, ${({ theme }) => theme.colors.background}, transparent); }
`;
...
const HighlightCard = styled(Link)`
  min-width: 360px; border-radius: 20px;
  background: ${({ theme }) => theme.colors.glass};
  backdrop-filter: blur(20px); -webkit-backdrop-filter: blur(20px);
  border: 1px solid ${({ theme }) => theme.colors.glassBorder};
  box-shadow:
  ...
  @media (max-width: 768px) { min-width: 290px; }
`;
const CardImg = styled.div`
  height: 220px; overflow: hidden;
  img { width: 100%; height: 100%; object-fit: cover; transition: transform 0.5s cubic-bezier(0.16,1,0.3,1); }
  ${HighlightCard}:hover & img { transform: scale(1.06); }
`;
const CardExcerpt = styled.div`
  height: 220px; padding: 24px;
  background: linear-gradient(160deg, ${({ theme }) => `${theme.colors.primary}22`} 0%, ${({ theme }) => `${theme.colors.secondary}15`} 100%);
  border-bottom: 1px solid ${({ theme }) => theme.colors.glassBorder};
  ...
  p {
    position: relative; z-index: 1; font-size: 13px; line-height: 1.7;
    color: ${({ theme }) => theme.colors.paragraph}; margin: 0;
    display: -webkit-box; -webkit-line-clamp: 5; -webkit-box-orient: vertical;
    overflow: hidden; opacity: 0.9;
  }
`;
const CardText = styled.div` padding: 20px 22px; `;
const CardCategory = styled.span`
  display: block; font-size: 11px; color: ${({ theme }) => theme.colors.primary};
...
const CardTitle = styled.h3`
  ...
  color: ${({ theme }) => theme.colors.headline}; line-height: 1.4;
`;
const CardSummary = styled.p`
  font-size: 0.825rem; color: ${({ theme }) => theme.colors.gray}; margin: 0; line-height: 1.6;
...
const GradientDivider = styled.div`
  ...
  background: linear-gradient(90deg, transparent, ${({ theme }) => theme.colors.primary}40, ${({ theme }) => theme.colors.secondary}40, transparent);
`;
  ...
  return (
    <Layout>
      ...
      <HeroSection>
        ...
        <Blob1 /><Blob2 /><Blob3 />
        <HeroContent>
          <HeroBadge><BadgeDot />기술과 사람, 그리고 이야기</HeroBadge>
          <HeroTitle>
        ...
        <RevealSection>
          {(visible) => (
            <div style={revealStyle(visible)}>
              <SectionHeader><SectionTitle>Apps</SectionTitle></SectionHeader>
              <AppCardGrid>
      ...
      <Section>
        <RevealSection>
          {(visible) => (
            <div style={revealStyle(visible)}>
              <SectionHeader><SectionTitle>Featured Stories</SectionTitle></SectionHeader>
              <FeaturedWrapper>
                <HorizontalScroll>
                  {posts.map(({ node }) => (
                    <HighlightCard key={node.fields.slug} to={node.fields.slug}>
                      {node.frontmatter.thumbnail ? (
                        ...
                        <CardExcerpt><p>{node.excerpt}</p></CardExcerpt>
                      )}
                      <CardText>
                        {node.frontmatter.category && <CardCategory>{node.frontmatter.category}</CardCategory>}
                        <CardTitle>{node.frontmatter.title}</CardTitle>
                        {node.frontmatter.summary && <CardSummary>{node.frontmatter.summary}</CardSummary>}
                      </CardText>
                    </HighlightCard>
                  ))}
                </HorizontalScroll>
      ...
      <Section id="latest">
        ...
        <RevealSection>
          {(visible) => (
            <GridContainer>
              {posts.map(({ node }, i) => (
                <GridItem key={node.fields.slug} $visible={visible} $delay={`${i * 0.08}`}>
                  <PostCard
